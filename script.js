...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchMenu() {
      ...
      try {
          ...
          if (!response.ok) throw new Error("قادر به ایجاد اتصال به سرور نیست");
          ...
          if (!dnesniItems.length && !nabidkaItems.length) {
              nabidkaList.innerHTML = '<p style="text-align:center;">منوی امروز هنوز وارد نشده است</p>';
              nabidkaBlock.hidden = false;
          }
    ...
    if (form && dateInput) {
      form.addEventListener('submit', (e) => {
        ...
        if (selectedDateTime - now < 24 * 60 * 60 * 1000) {
          e.preventDefault();
          alert("پوزش می‌خواهیم، ​​اما رزروها را حداقل از ۲۴ ساعت قبل می‌پذیریم. لطفاً زمان دیگری را انتخاب کنید");
        }
      });
    }
  };
